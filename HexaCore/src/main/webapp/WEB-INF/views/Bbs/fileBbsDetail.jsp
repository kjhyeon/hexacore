...
<script type="text/javascript">
	function fileBbsContent_modify(){
		location.href="./fileBbsModify.do?seq="+${seq.seq};
	}
	
	function file_Del(){
		location.href="./fileDel.do?seq="+${seq.seq};
	}
	
	function fileBbsReplyWrite(){
		location.href="./fileBbsReplyInsert.do?seq="+${seq.seq};
	}
	...
	function deleteComment(seq) {
		location.href="./fileCommentDelete.do?seq="+seq+"&parent_seq="+$("#seq").val();
	}
	
	function goPage(page){
		var location = "./fileBbsDetail.do?seq="+${seq.seq}+"&page="+page; 
...
<sec:authentication property="principal.username" var="sessionId"/>
	...
	<div class="Detail_container">
		<form action="./fileDel.do" method="post">
        			...
        			<td>${seq.views}</td>
     		 		...
     		 		<td>
     		 			<c:forEach items="${list}" var="files" varStatus="n">
     		 				<a href="./fileDownload.do?name=${files.name}" >${files.ori_name}</a> | ${files.f_size }byte
     		 			</c:forEach>
     		 		</td>
     		 	</tr>
      			<tr>
	    ...
	    <div class="Detail_form_group">        
	      <div class="Detail_form_Button">
	        ...
	        <c:if test="${(sessionId eq seq.id) || auth eq true}">
	        	<input id="Detail_Delete" type="submit" class="btn btn-default" value="삭제" onclick="file_Del()">
	        	<input id="Detail_Modify" type="button" class="btn btn-default" onclick="fileBbsContent_modify()" value="수정">
	        </c:if>
	        	<input id="Detail_Reply" type="button" class="btn btn-default" onclick="fileBbsReplyWrite()" value="답글">
	      </div>
	    </div>
	   </form>
	</div>
	...
	<form action="./fileCommentWrite.do" method="post">
